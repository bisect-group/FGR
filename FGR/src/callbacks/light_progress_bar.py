    ...
    for index in range(len(strings)):
        ...
        while len(string) > 0:
            ...
            while (
                ...
                and colors[index][color_index][0] - position < string_end - padding
            ):
    ...
    def update(self, cur_state: float) -> None:
        ...
        if self.cur_state >= self.eta_threshold:
            self.est_finish_time = (
                self.start_time + (time.time() - self.start_time) / self.cur_state
            )
...
class LightProgressBar(ProgressBarBase):
    ...
    def step(self, part: str, batch_idx: int, total_batches: int, *args: Any) -> None:
